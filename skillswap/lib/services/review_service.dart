...
class ReviewService {
  ...
  Future<void> submitReview(ReviewModel review) async {
    try {
      ...
      bool canReview = await canStudentReview(
        review.sessionId,
        review.studentId,
      );
      if (!canReview) {
        throw Exception(
          'You cannot review this session. Make sure the session is completed and you have made payment.',
        );
      }

      // Check if student has already reviewed this session
      bool hasReviewed = await hasStudentReviewed(
        review.sessionId,
        review.studentId,
      );
      if (hasReviewed) {
